...
namespace ServerConnection
{
	...
	bool Payload::info(Information& information)
	{
	...
	int Payload::send(const char opcode, char out[QUERY_BASIC_PACKET_LENGTH])
	{
	...
	bool Payload::recvInfo(Information& info, char in[QUERY_BASIC_PACKET_LENGTH])
	{
	...
	bool Payload::assemble(const char opcode, char out[QUERY_BASIC_PACKET_LENGTH])
	{
		out[0] = 'S';
		out[1] = 'A';
		out[2] = 'M';
	...
	Payload::Payload(std::string ip, const unsigned short port, long timeout) : server{ 0 }
	{
		...
		server.sin_addr.s_addr = inet_addr(ip.c_str()); // InetPton(AF_INET, (wchar_t*)ip.c_str(), &server.sin_addr.s_addr); //
	}
